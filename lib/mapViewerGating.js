 ...
 * Fail-open if `subscription.isPaidActive` is not explicitly `false`.
 * @param {import("next").NextApiRequest} req
...
export async function getExcludeViewerMighty(req, collection) {
  ...
  const paid = viewerRecord.subscription?.isPaidActive;
  if (paid !== false) return { excludeMongoId: null, excludeMightyId: null };
