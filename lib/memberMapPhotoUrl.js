...
const LEGACY_PHOTO_API_PATH = "/api/member-legacy-photo";

function isValidAirtableRecordId(recordId) {
  return typeof recordId === "string" && /^rec[a-zA-Z0-9]{10,}$/.test(recordId.trim());
}

function legacyMemberPhotoProxyPath(recordId) {
  const id = String(recordId).trim();
  return `${LEGACY_PHOTO_API_PATH}?recordId=${encodeURIComponent(id)}`;
}
