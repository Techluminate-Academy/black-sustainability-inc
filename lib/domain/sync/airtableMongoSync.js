    ...
    await session.withTransaction(async () => {
      ...
      for (let start = 0; start < resolved.length; start += BATCH_SIZE) {
        ...
        for (const { item, doc } of resolved.slice(start, start + BATCH_SIZE)) {
          ...
          // Scheduled imports only fill missing profile values. Billing and sync metadata remain live.
          if (doc) {
            const profileKeys = ['firstName', 'lastName', 'email', 'location', 'bio', 'avatarUrl', 'industry', 'accountCreatedAt'];
            for (const key of profileKeys) if (nonblank(getPath(doc, key))) delete fields[key];
            const hasLocation = ['latitude', 'longitude', 'geo'].some(key => nonblank(doc[key]));
            const differentCity = nonblank(doc.location) && nonblank(item.fields.location) && doc.location !== item.fields.location;
            if (hasLocation || differentCity) {
              delete fields.latitude; delete fields.longitude; delete fields.geo;
            }
            if (hasLocation) delete fields.location;
          }
