...
async function ensureIdentityIndexes(collection) {
  // Unique-index builds are also a duplicate audit: existing collisions stop the run.
  // Never delete or auto-merge an existing member to make an index succeed.
  await collection.createIndex({ mightyId: 1 }, {
    unique: true, partialFilterExpression: { mightyId: { $type: 'number', $gt: 0 } },
  });
  await collection.createIndex({ 'airtable.recordId': 1 }, {
    unique: true, partialFilterExpression: { 'airtable.recordId': { $type: 'string', $gt: '' } },
  });
  await collection.createIndex({ email: 1 }, {
    name: 'sync_unique_email', unique: true, collation: EMAIL_COLLATION,
