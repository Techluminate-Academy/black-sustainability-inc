...
const sampleRow: RowData = {
  ...
  firstName: "John",
  lastName: "Doe",
  organizationName: "Test Sustainability Organization",
  website: "https://example.com",
  bio: "This is a test member profile for testing the batch upload functionality. This member is interested in sustainable practices and community development.",
  identification: "Black/African-American",
  gender: "Male",
  phoneUS: "(555) 123-4567",
  phoneNonUS: "",
  primaryIndustry: "☀️ Alternative Energy",
  additionalFocus: "🌾 Agriculture/Sustainable Food Production / Land Management, 💧Water",
  naicsCode: "237990",
  affiliatedEntity: "Test Affiliated Organization",
  address: "123 Main Street, Chicago, IL, United States",
  nearestCity: "Chicago",
  ...
  latitude: "41.8781",
  longitude: "-87.6298",
  memberLevel: "Core Member",
  payingMember: "Yes",
  equityMember: "No",
  membershipNotes: "Test submission - please review",
  sendPaymentEmail: "No"
};
...
export default function BatchUploadPage() {
  // Start with a sample row for testing
  const [rows, setRows] = useState<RowData[]>([{ ...sampleRow }]);
  ...
  const handleSubmit = async () => {
    ...
    const loadingToast = toast.loading(`Submitting ${rows.length} member(s)...`);
    ...
    try {
      ...
      let successMessage = data.message || `Saved ${rows.length} member(s) for review!`;
      if (data.warning) {
      ...
      // Reset to sample row for easy testing (or clear with initialRow)
      setRows([{ ...sampleRow }]);
    } catch (error: any) {
      toast.dismiss(loadingToast);
